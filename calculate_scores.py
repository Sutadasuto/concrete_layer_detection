...
def main(args):
    ...
    paths = data.create_image_paths_from_text(args.dataset_paths)
    predicted_paths = data.create_image_paths_from_text(args.prediction_paths)
    ...
    for i, path in enumerate(paths.transpose()):
        ...
        y = np.where(gt >= 0.5, 1.0, 0.0)
        values, counts = np.unique(y, return_counts=True)
        try:
            n_black = counts[np.where(values == 0.0)][0]
        except IndexError:  # There are no black pixels in the image
            n_black = 0
        try:
            n_white = counts[np.where(values == 1.0)][0]
        except IndexError:  # There are no white pixels in the image
            n_white = 0
        if n_black < n_white:  # Cracks are black instead of white
            y = 1.0 - y  # Adjust to prediction (white cracks on black background)
