...
def main(args):
    ...
    paths = data.create_image_paths(args.images_path, args.annotations_path)
    # Ensure that annotations are binary and uint8
    ...
    all_image_paths = sorted([os.path.join(args.images_path, f) for f in os.listdir(args.images_path) if
                              not f.startswith(".") and f.endswith(".tiff") and not os.path.exists(
                                  os.path.join(args.annotations_path, f.replace(".tiff", ".png")))],
                             key=lambda f: f.lower())

    print("Predicting unlabeled images using the weights with minimum validation loss...")
    data.save_predictions(model, all_image_paths, results_train_dir)
    if args.epochs > 0:  # Save the best weights into the predictions folder if the model was trained
        os.replace('%s_best.hdf5' % args.model, os.path.join(results_train_dir, '%s_best.hdf5' % args.model))

    # Evaluate the model if a path to ground truths was provided
    if args.gt_path:
        print(
            "Validating the weights with minimum validation loss using the ground truths provided in %s" % args.gt_path)
        paths = data.create_image_paths(args.images_path, args.gt_path)
        # Ensure that ground truths are binary and uint8
        data.annotation_thresholding(paths[1, :])
        data.evaluate_model_on_paths(model, paths, results_validation_dir, args)
...
def parse_args(args=None):
    ...
    parser.add_argument("-ap", "--annotations_path", type=str,
                        help="Path to the directory containing the annotations used to train.")
    ...
    parser.add_argument("-da", "--use_da", type=str, default="False",
                        help="If 'True', training will be done using data "
